...
class AgileStoreLocator_Activator {
	...
	public static function add_basic_tables() {
		...
		//	Products
		$sql 	= "SELECT count(*) as c FROM information_schema.COLUMNS WHERE TABLE_NAME = '{$prefix}stores' AND COLUMN_NAME = 'product' AND TABLE_SCHEMA = '{$database}'";
		$result = $wpdb->get_results($sql);
		
		if($result[0]->c == 0) {
			$wpdb->query("ALTER TABLE {$prefix}stores ADD COLUMN `product` text;");
		}
